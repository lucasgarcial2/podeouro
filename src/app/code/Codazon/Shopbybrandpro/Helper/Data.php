...
class Data extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function getProductCounts($attributeCode, $optionIds)
    {
        $values = $optionIds;

        $connection = $this->_resourceConnection->getConnection();
        $select = $connection->select()
            ->from(['e' => $connection->getTableName('catalog_product_entity')])
            ->joinLeft(
                ['at_manufacturer' => $connection->getTableName('catalog_product_entity_int')],
                'at_manufacturer.entity_id = e.entity_id AND at_manufacturer.attribute_id = (
                    SELECT attribute_id FROM ' . $connection->getTableName('eav_attribute') . ' WHERE attribute_code = \'' . $attributeCode . '\' LIMIT 1
                ) AND (at_manufacturer.store_id = 0 OR at_manufacturer.store_id IS NULL)',
                ['manufacturer_value' => new \Zend_Db_Expr('IFNULL(at_manufacturer.value, 0)')]
            )
            ->where('IFNULL(at_manufacturer.value, 0) IN (?)', $values)
            ->columns(['product_count' => new \Zend_Db_Expr('COUNT(*)')])
            ->group('manufacturer_value');
